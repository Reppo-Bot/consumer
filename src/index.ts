...
export const cleanCommands = async(handler: Handler | typeof axios, commands: Command[], appId: string, serverId: string, token: string) => {
    ...
    discordCommands.filter((discordCommand: DiscordCommand) => names.includes(discordCommand.name))
    .forEach(async (discordCommand: DiscordCommand) => {
        const result = await discordCommandsCall(handler, 'delete', `${BASE_URL}/${appId}/guilds/${serverId}/commands/${discordCommand.id}`, null, token)
        if(result.status === 200 || result.status === 201) {
            console.log(`Removed command ${discordCommand.name}`)
        } else {
            console.log(`Failed to remove command ${discordCommand.name}`)
            return false
        }
    })
    return true
